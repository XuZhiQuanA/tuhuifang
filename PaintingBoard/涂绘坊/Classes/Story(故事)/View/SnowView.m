...
- (void)setup {
    ...
    self.emitterLayer.emitterShape    = kCAEmitterLayerCuboid;                            // 直线粒子发射器
    self.emitterLayer.emitterMode     = kCAEmitterLayerSurface;                         // ?????
    //    self.emitterLayer.emitterSize     = self.frame.size;                                // 发射区域
    self.emitterLayer.emitterSize     = CGSizeMake(self.frame.size.width * 5, self.frame.size.height * 5);
...
- (void)showSnow {
    ...
    // 粒子生命周期
    snowflake.lifetime = (_lifetime > 0 ? _lifetime : 140);
...
//        self.snowImage = [UIImage imageNamed:@"record_animate_02"];
        self.snowColor = [UIColor blackColor];
        self.lifetime  = 30.f;
        self.alpha     = 0.f;
        
        UIImageView *snowAlpha = [[UIImageView alloc] initWithFrame:self.bounds];
        snowAlpha.image        = [UIImage imageNamed:@"alpha"];//record_animate_02
//        snowAlpha.image        = [UIImage imageNamed:@"record_animate_02"];
